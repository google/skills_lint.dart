...
6. **Artifact & Teardown**: Grade the outputs and generate a Markdown artifact (e.g., `<skill>_eval_results.md`) containing the metadata (including the model used, effort level, and active agent profile/configuration name such as `"reidbaker-agent"` or `"Main Agent"` when available), pass/fail rationale, and raw diffs/stdout. If an isolated worktree was used (Strategy B), clean it up via `git worktree remove --force <skill-name>-workspace`.
