...
3. **Determine Agent Configuration**: By default, the evaluation runner inherits the agent configuration/profile used in the chat where `/run-evals` was triggered (`TypeName: self`). If `agent_config` is explicitly present in an `evals.json` entry, use it as a fallback for legacy compatibility.
4. **Orchestrate & Isolate**:
   ...
   - **Spawn Subagent(s)**: By default, run an Integration Test by spawning a single **With-Skill** subagent using the selected isolation strategy and the inherited agent configuration (`TypeName: self`, or fallback `TypeName: <agent_config>` if explicitly specified in `evals.json`).
...
6. **Artifact & Teardown**: Grade the outputs and generate a Markdown artifact (e.g., `<skill>_eval_results.md`) containing the metadata (including the model used, effort level, and agent configuration when available), pass/fail rationale, and raw diffs/stdout. If an isolated worktree was used (Strategy B), clean it up via `git worktree remove --force <skill-name>-workspace`.
