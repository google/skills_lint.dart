...
3.  **Metrics**: Run `dart run cognitive_complexity --fail-threshold <N> lib test`, where `<N>` is the `--fail-threshold` value configured in `.github/workflows/skills_lint_workflow.yaml`, and ensure there are zero issues. This checks for cognitive complexity.
