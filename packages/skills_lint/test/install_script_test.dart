...
String _getPackageRoot() {
  ...
  while (dir.path != '/' && dir.path.isNotEmpty) {
    ...
    if (pubspec.existsSync() && pubspec.readAsStringSync().contains('name: skills_lint')) {
      return dir.path;
    }
    dir = dir.parent;
  }
  // Fallback to searching subdirectories
  final subdir = Directory(p.join(currentPath, 'tool', 'skills_lint'));
...
Future<void> _createMockRelease({
  ...
  final binaryName = 'skills_lint-$target';
  final archiveName = 'skills_lint-$target.tar.gz';
...
Future<void> _runInstallScriptTest({
  ...
  final installedFile = File(p.join(installDir.path, 'skills_lint'));
